...
Absorbance_all<-time_45$A-time_0$A
...
#for all replicates (not just technical replicates) used in figure 3b
Absorbance_final_2<-Absorbance_final[-c(6)]
Absorbance_final_3<-Absorbance_final_2[-c(1:6, 76:81, 154, 203:205, 215),]
BSA_reps<-bsa_72[-c(1:8, 78:95, 165:182, 252:684),]
Absorbance_final_3<-cbind(Absorbance_final_3, BSA_reps$Well, BSA_reps$A)
Absorbance_4<-merge(Absorbance_final_3, BSA_reps, by="Label", all=TRUE)
delete.na <- function(DF, n=0) {
  DF[rowSums(is.na(DF)) <= n,]
}
Na_removed_all<-delete.na(Absorbance_4)
protein_content_na_removed_all<-(((3000*Na_removed_all$A)-2500)/1000/1000)*43 #mg
...
u_plot<-ggplot(data=results_unique,(aes(x=protein_b, y=CS_activity, color=trt_list)))+geom_point()+theme_bw()+geom_smooth(method="lm", se=FALSE, formula = y ~ x ) + ylab("CS Activity (nmol/(mg*min))") +xlab("Protein (mg/uL)")
u_plot #figure 2a

u2_plot<-ggplot(data=results_unique,(aes(x=protein_b, y=CS_activity)))+geom_point()+theme_bw()+geom_smooth(method="lm", se=TRUE, formula = y ~ log(x) ) + ylab("CS Activity (nmol/(mg*min))") +xlab("Protein (mg/uL)")
u2_plot #figure 2b

u3_plot<-ggplot(data=CS_all_na_removed, (aes(x=CS_all_na_removed$protein_content_na_removed_all, y=CS_all)))+geom_point()+theme_bw()+geom_smooth(method="lm", se=TRUE, formula = y ~ x ) + ylab("CS Activity (nmol/(mg*min))") +xlab("Protein (mg)")
u3_plot #figure 2c
